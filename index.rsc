0:{"page:/":"$L1","layout:/":"$L2","route:/":"$L3","__route":"route:/","__interceptionContext":null,"__layoutIds":["layout:/"],"__rootLayout":"/","__bfcacheSegmentIdentities":{"layout:/":"[\"layout\",\"layout:/\",\"root-boundary:/\",\"\"]","page:/":"[\"page\",\"page:/\",\"root-boundary:/\",\"\"]"},"__srcPage":["page"],"__layoutFlags":{"layout:/":"s"},"__artifactCompatibility":{"schemaVersion":1,"graphVersion":"app-route-graph:1177fd80f83fa7b0","deploymentVersion":"0d2e2331-65ac-4eb6-8c4a-6379642ff04e","appElementsSchemaVersion":1,"rscPayloadSchemaVersion":1,"rootBoundaryId":"/","renderEpoch":null}}
...
- Interpolation/rank module: seek a certified rank deficit beyond the current guaranteed surplus `9117`.
- Verifier: replay the incumbent checker after replacing `kappa` by `kappa'` and adding the single identity `6693082347−6693073230−9116=1`.
16:T5f2,Set $D=15129$. The number of monomials $X^iY^jZ^z$ satisfying $i+j+87z\leq D$ is
...
Thus the reduced interpolant again has nonzero $Z$-derivative, contradicting minimality. Therefore $A$ is squarefree.17:T5c1,Write
...
and hence has at most $15042$ roots.18:T74f,Put
...
and hence has at most $15042$ roots.1c:T691,Put
...
Complete deletion is therefore impossible at density at least $\varepsilon$. The surviving core has the asserted minimum degrees; its lines are regular primitive identity lines, and the derivative-zero incidences were deleted.1d:T66e,Let $X$ and $S$ be the point and line vertex sets of a connected core component. Put $a=|X|/M$, let $\ell=|S|/\Lambda$, and write $m_L=|X\cap L|$. Ordered-pair counting gives
...
so the integral point count of every component is at least $734949122$.1e:T5ba,Choose a point $x_0$ in a retained component and translate it to the origin. Put
...
The candidate score is therefore smaller by $1187$. This arithmetic records compatibility with the zero-loss algebraic conclusion but does not establish the candidate's other dependencies.22:T5c4,Put $p=147457$, $d=87$, $N=p^2$, and $c=dp=12828759$. Let $Q_1,\ldots,Q_r$ be the polynomials counted by $R_K(f)$. For $i\ne j$, the nonzero polynomial $Q_i-Q_j$ has total degree at most $87$, so it has at most $c$ zeros in $\mathbb F^2$.
...
so the hypothesis does not hold there.23:T40e,For each polynomial $Q$, put
...
Every unexplained point lies outside $T$, where $\widetilde f=f$. If more than $uN$ such points were $\beta$-good, their retained accepted incidences would give the overwritten table acceptance greater than $\beta u\ge\sigma$. The weak-recovery hypothesis would then produce a polynomial with at least $H$ agreements with $\widetilde f$, contradicting the preceding cap.24:T541,For a point set $S\subseteq\mathbb F^2$ of density $a$, let $Y=|S\cap L|$ for a uniformly random affine line $L$. Every point lies on $p+1$ lines, and every ordered pair of distinct points determines one line. Hence
...
contradicting the hypothesis.25:T455,Put
...
its integral agreement count is at least $781092078$.26:T6c6,The $3\%$ threshold is
...
show that the union bound over all these polynomials is strictly below $1$. Hence some such table has acceptance greater than $\sigma_{94}$ but no total-degree-at-most-$87$ polynomial with $H_3$ agreements. This table refutes the stated implication.27:T43f,In HKSS v1 Lemma $B.4$, the printed hypothesis is
...
whose sum is $\eta_1+4\beta$, not the claimed $\eta_1+3\beta$. Thus neither written numerical inference follows from the printed hypotheses and displayed case bounds. This refutes those inferences, not the possibility of a separate repair.28:T7c2,The checker uses exact rational arithmetic. Put
